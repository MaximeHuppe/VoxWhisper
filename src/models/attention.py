...
class PositionalEncoding3D(nn.Module):
    ...
    and not generalise to new resolutions), three independent parameter vectors
    along D, H, W are summed by broadcasting:

        pos[d, h, w] = pos_d[d] + pos_h[h] + pos_w[w]

    This keeps the parameter count at ``3 × grid_size × embed_dim`` and allows
    interpolation to arbitrary runtime sizes when the spatial resolution differs
    from the trained grid (e.g. when evaluating with a different patch size).

    Parameters
    ----------
    embed_dim            : Feature dimension C.
    d_size, h_size, w_size : Spatial grid size at training time (bottleneck).
    """
