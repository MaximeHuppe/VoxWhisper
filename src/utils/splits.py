...
def create_or_load_splits(config: Mapping) -> dict[str, list[str]]:
    ...
    Requires ``splits.enabled: true`` in the config.
    """
    if not config["splits"].get("enabled", False):
        raise RuntimeError("create_or_load_splits called but splits.enabled is false")
