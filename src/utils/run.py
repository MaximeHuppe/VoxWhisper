...
``data.paths.cache`` stays for shared artifacts (prompt embeddings).
"""
...
from src.utils.config import ensure_dir, get_project_root, resolve_path
...
def resolve_run_name(config: Mapping[str, Any], name_override: Optional[str] = None) -> str:
    """Return validated run name: CLI ``--name`` wins over ``training.run_name``."""
    raw = name_override if name_override is not None else config.get("training", {}).get("run_name", "baseline")
    return validate_run_name(str(raw))


def dataset_name_from_config(config: Mapping[str, Any]) -> str:
    """Leaf of ``data.paths.processed`` used as the dataset folder (e.g. ``processed_T1_FA``)."""
...
def run_family_dir(config: Mapping[str, Any], name_override: Optional[str] = None) -> Path:
    """``{runs}/{dataset}/{run_name}`` — parent of timestamped children."""
    return runs_root(config) / dataset_name_from_config(config) / resolve_run_name(config, name_override)
...
def is_legacy_run_dir(path: Path) -> bool:
    """True when checkpoints sit flat in a pre-layout folder (no timestamp child)."""
    return path.is_dir() and (path / _LATEST_CKPT).exists()
...
def write_meta(
    ...
    *,
    ...
    meta: dict[str, Any] = {
        ...
        "modalities": dict(config.get("data", {}).get("modalities", {})),
    }
...
def create_run_dir(
    ...
    *,
    ...
    run_name = resolve_run_name(config, name_override)
    ...
    run_dir = ensure_dir(run_family_dir(config, name_override) / stamp)
...
def create_or_resume_run(
    ...
    *,
    ...
    Priority
    --------
    1. Explicit ``run_dir`` — created if absent; config/meta written if missing.
    2. ``--resume`` → newest timestamp child (or legacy flat family) with latest.pt.
    ...
    family = run_family_dir(config, name_override)

    if resume:
        latest = find_latest_run_dir(family)
        if latest is not None:
            return latest
        if is_legacy_run_dir(family):
            return family
        print(f"Warning: --resume requested but no prior run found under {family} — starting a new run")

    return create_run_dir(config, name_override=name_override,
                          config_path=config_path, seed=seed, argv=argv)
...
def resolve_run_dir_for_eval(
    ...
    *,
    ...
    family = run_family_dir(config, name_override)
    latest = find_latest_run_dir(family)
    if latest is None and is_legacy_run_dir(family):
        return family
    return latest
...
def find_checkpoint_for_eval(
    ...
    *,
    ...
    eval_run = resolve_run_dir_for_eval(config, run_dir=run_dir, name_override=name_override)
